...
<div class="rounded-2xl border-4 border-s-black bg-white p-6 shadow-[6px_6px_0px_0px_rgba(26,26,26,1)]">
    <div class="mb-6 flex items-center justify-between border-b-2 border-gray-100 pb-4">
        <h3 class="text-xl font-black uppercase tracking-tight text-s-black">Course Visibility</h3>
        <span class="rounded-lg border-2 border-s-black px-2 py-1 text-[10px] font-black uppercase tracking-widest bg-gray-100">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {#each STAGES as stage}
                <button
                    ...
                    {selectedState === stage ? 'bg-s-black text-white -translate-y-1 shadow-[4px_4px_0px_0px_rgba(145,245,173,1)]' : 'bg-white text-s-black hover:bg-gray-50'}"
                >
                    <span class="text-2xl">{stageMeta[stage].icon}</span>
                    <span class="mt-1 text-xs font-black uppercase tracking-tighter">{stageMeta[stage].label}</span>
                </button>
            {/each}
        </div>
        ...
        {#if enableTimer}
            <div class="rounded-xl border-2 border-dashed border-gray-300 p-4">
                <label class="mb-2 flex cursor-pointer items-center gap-2">
                    ...
                    />
                    <span class="text-sm font-bold uppercase text-s-black">Optionally Schedule State Change</span>
                </label>

                {#if showTimer}
                    <div transition:slide class="mt-4 space-y-2">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-400" for="release_date">Apply change at:</label>
                        <input
                            id="release_date"
                            type="datetime-local"
