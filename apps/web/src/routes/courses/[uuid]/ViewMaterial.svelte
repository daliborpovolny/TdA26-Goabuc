...
<div class="space-y-2 rounded-xl bg-[#91F5AD] p-4">
	<button class="cursor-pointer w-[100%] text-left text-2xl font-bold" type="button" onclick={() => (collapsed = !collapsed)}>
		{material.name}
	</button>
	{#if !collapsed}
		<div>
			...
			{:else if material.type === 'url'}
				<a target="_blank" href={material.url} class="border border-[#1a1a1a] rounded-xl p-1 text-xl w-fit bg-[#49B3B4] hover:bg-[#6DD4B1] text-white">
					View
				</a>
			{/if}
		</div>
