...
<nav class="border-b-4 border-s-black bg-s-blue-darker text-white shadow-md">
    <div class="mx-auto flex max-w-7xl flex-col md:flex-row md:items-center">
        ...
        <div class={`flex flex-col items-center md:flex md:flex-row md:flex-1 md:justify-start ${show ? 'flex' : 'hidden'}`} transition:slide={{ duration: 300 }}>
            ...
            {#if auth.user}
                ...
                <button 
                    ...
                    class="w-full px-6 py-4 text-left font-black uppercase tracking-widest text-red-300 hover:bg-red-500 hover:text-white md:ml-auto md:w-auto md:py-2 md:text-sm"
                >
                    Logout
                </button>
            {:else}
